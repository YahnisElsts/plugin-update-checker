...
if ( !class_exists('Puc_v4p3_StateStore', false) ):

	class Puc_v4p3_StateStore {
		/**
		...
		/**
		 * @var Puc_v4p3_Update|null Cached update.
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(Puc_v4p3_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( Puc_v4p3_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(Puc_v4p3_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = Puc_v4p3_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
